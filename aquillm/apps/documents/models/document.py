...
class Document(models.Model):
    ...
    def filter(*args, **kwargs) -> List[DocumentChild]:
        ...
        return functools.reduce(
            lambda l, r: l + r,
            [list(x.objects.filter(*args, **kwargs)) for x in doc_types],
        )
    ...
    def save(self, *args, dont_rechunk=False, **kwargs):
        ...
        database_alias = kwargs.get("using") or router.db_for_write(
            document_model, instance=self
        )
        update_fields = kwargs.get("update_fields")
        requested = None if update_fields is None else set(update_fields)
        writes_text = (
            self._state.adding
            or requested is None
            or bool({"full_text", "full_text_hash"} & requested)
        )
        writes_collection = (
            self._state.adding
            or requested is None
            or bool({"collection", "collection_id"} & requested)
        )
        with transaction.atomic(using=database_alias):
            ...
            if not self._state.adding and self.pkid is not None:
                ...
                if (
                    ...
                    child_figures = getattr(self, "child_figures", None)
            ...
            if writes_text:
                ...
                content_changed = (
                    previous is None
                    or previous["full_text_hash"] != self.full_text_hash
                )
                if requested is not None:
                    requested.update({"full_text", "full_text_hash"})
            elif previous is not None:
                self.full_text_hash = previous["full_text_hash"]
            if content_changed and not dont_rechunk:
                self.ingestion_complete = False
                if requested is not None:
                    requested.add("ingestion_complete")
            save_kwargs = dict(kwargs, using=database_alias)
            if requested is not None:
                save_kwargs["update_fields"] = sorted(requested)
            super().save(*args, **save_kwargs)
            if content_changed and not dont_rechunk:
                document_id, source_hash, pkid = (
                    str(self.id),
                    self.full_text_hash,
                    int(self.pkid),
                )
                ...
                transaction.on_commit(publish_chunks, using=database_alias, robust=True)
    ...
    def move_to(self, new_collection, *, actor):
        ...
        with transaction.atomic(using=database_alias):
            current = (
                type(self)
                ._base_manager.using(database_alias)
                .select_for_update()
                .get(pkid=self.pkid, id=self.id)
            )
            ...
            current.save(
                dont_rechunk=True, update_fields=["collection"], using=database_alias
            )
