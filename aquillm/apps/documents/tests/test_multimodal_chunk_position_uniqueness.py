...
def test_create_chunks_avoids_text_image_position_collision(
    ...
    monkeypatch.setattr(
        "aquillm.models.create_chunks.delay", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
        "apps.documents.tasks.chunking.get_channel_layer", lambda: _DummyChannelLayer()
    )
    monkeypatch.setattr(
        "apps.documents.services.chunk_progress.get_channel_layer",
        lambda: _DummyChannelLayer(),
    )
    monkeypatch.setattr(
    ...
    # Keep text shorter than CHUNK_SIZE (default 2048) so the first text chunk is
    # (0, 116).
    text = "x" * 116
    doc = ImageUploadDocument.objects.create(
        ...
        image_file=SimpleUploadedFile(
            "collision.png", b"\x89PNG....", content_type="image/png"
        ),
    )
    ...
    text_chunk = next(
        chunk for chunk in chunks if chunk.modality == TextChunk.Modality.TEXT
    )
    image_chunk = next(
        chunk for chunk in chunks if chunk.modality == TextChunk.Modality.IMAGE
    )
