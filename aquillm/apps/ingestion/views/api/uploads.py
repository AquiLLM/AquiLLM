...
def ingest_pdf(request):
    ...
    try:
        ...
        logger.error("obs.ingest.upload_view_error", error_type=type(e).__name__, error=e.message)
        return JsonResponse({"error": e.message}, status=200)
    except DatabaseError as e:
        logger.error("obs.ingest.upload_view_error", error_type=type(e).__name__, error=str(e))
