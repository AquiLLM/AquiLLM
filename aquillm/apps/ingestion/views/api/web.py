...
def ingest_webpage(request):
    ...
    try:
        ...
        if not url or not collection_id:
            logger.warning("obs.ingest.webpage_missing_params", url=url, collection_id=collection_id)
            return JsonResponse({"error": "Missing url or collection_id"}, status=400)

        if not url.startswith(("http://", "https://")):
            logger.warning("obs.ingest.webpage_invalid_url", url=url)
            return JsonResponse({"error": "Invalid URL format. Must start with http:// or https://"}, status=400)

        try:
            collection = Collection.objects.get(pk=collection_id)
        except Collection.DoesNotExist:
            logger.warning("obs.ingest.webpage_collection_not_found", collection_id=collection_id)
            return JsonResponse({"error": "Collection not found"}, status=404)

        if not collection.user_can_edit(request.user):
            logger.warning(
                "obs.ingest.webpage_permission_denied",
        ...
        try:
            ...
            logger.error(
                "obs.ingest.webpage_dispatch_error",
                url=url,
        ...
        logger.warning("obs.ingest.webpage_invalid_json")
        return JsonResponse({"error": "Invalid JSON payload"}, status=400)
    except PermissionDenied as e:
        return JsonResponse({"error": str(e)}, status=403)
    except Exception as e:
        logger.error(
            "obs.ingest.webpage_error",
            error=str(e),
