...
class ToolUseRetryTests(SimpleTestCase):
    ...
    def test_retries_required_tool_choice_when_model_only_promises_to_retrieve(self):
        ...
        self.assertEqual(llm.calls[1]["tool_choice"]["type"], "any")
        self.assertEqual(
            llm.calls[1]["stream_message_uuid"], llm.calls[0]["stream_message_uuid"]
        )
        self.assertEqual(updated[-1].tool_call_name, "_test_document_ids")
        self.assertNotIn("I'll retrieve", updated[-1].content)

    def test_required_tool_cutoff_falls_back_without_second_model_generation(self):
        llm = _FakeLLMInterface(
            [
                LLMResponse(
        ...
        self.assertEqual(llm.calls[0]["tool_choice"]["type"], "any")
        self.assertEqual(llm.calls[0]["max_tokens"], 256)
        self.assertEqual(llm.calls[0]["thinking_budget"], 0)
    ...
    def test_zero_tool_step_token_caps_use_safe_mechanical_budget(self):
        llm = _FakeLLMInterface(
            [
                LLMResponse(
                    text=None,
    ...
    @patch.dict(
        "os.environ",
        {
            "LLM_TOOL_STEP_MAX_TOKENS": "4096",
            "LLM_TOOL_CALL_RETRY_MAX_TOKENS": "4096",
        },
    )
    def test_oversized_tool_step_caps_are_clamped_to_mechanical_budget(self):
        llm = _FakeLLMInterface(
            [
                LLMResponse(
                    text=None,
    ...
    def test_required_document_tool_turn_falls_back_to_vector_search_when_model_only_thinks(
        self,
    ):
        llm = _FakeLLMInterface(
            [
    ...
    def test_required_tool_request_retries_when_forced_retry_still_promises_to_search(
        self,
    ):
        llm = _FakeLLMInterface(
            [
        ...
        self.assertEqual([call["max_tokens"] for call in llm.calls], [256, 256, 256])
        self.assertEqual([call["thinking_budget"] for call in llm.calls], [0, 0, 0])
        self.assertEqual(updated[-1].tool_call_name, "_test_document_ids")
        self.assertNotIn("could not complete", updated[-1].content)
    ...
    def test_auto_tool_followup_retries_plain_answer_when_tool_retry_stays_interim(
        self,
    ):
        llm = _FakeLLMInterface(
            [
        ...
        convo = Conversation(
            ...
            messages=[
                ...
                ToolMessage(
                    ...
                    for_whom="assistant",
                    result_dict={
                        "result": "The paper discusses calibration, confidence, utility, and factuality."
                    },
                ),
            ],
        )
...
class CutoffContinuationTests(SimpleTestCase):
    ...
    def test_continues_cutoff_response_before_compact_fallback(self):
        ...
        convo = Conversation(
            ...
            messages=[
                UserMessage(
                    content="Give me a detailed patent-analysis implementation outline."
                )
            ],
        )
