...
def test_exact_unicode_spans_validate_against_full_source():
    ...
    prepared = PreparedEvidence(item, (span,), fingerprint_prepared_evidence(item, (span,)), 2, "partial")
    assert prepared.spans[0].text == "😀β"
    assert prepared.source_coverage == "partial"
    with pytest.raises(ValueError):
        PreparedEvidence(item, (SourceSpan(7, "revision-a", 1, 3, "😀"),), "packet-a", 2, "partial")
    with pytest.raises(ValueError):
        PreparedEvidence(item, (SourceSpan(7, "revision-b", 1, 3, "😀β"),), "packet-a", 2, "partial")


def test_complete_coverage_cannot_hide_a_missing_tail():
    item = source()
    with pytest.raises(ValueError):
        PreparedEvidence(item, (SourceSpan(7, "revision-a", 0, 3, "A😀β"),), "packet-a", 2, "complete")
    full = SourceSpan(7, "revision-a", 0, len(item.text), item.text)
    fingerprint = fingerprint_prepared_evidence(item, (full,))
    assert PreparedEvidence(item, (full,), fingerprint, 4, "complete").source_coverage == "complete"
...
def test_prepared_fingerprint_rejects_mutated_text_and_reordered_spans():
    ...
    assert PreparedEvidence(original, spans, fingerprint, 2, "partial").evidence_fingerprint == fingerprint
