...
def run_zotero_library_sync(user_id: int, library_config: Optional[dict] = None) -> dict:
    ...
    logger.info("obs.zotero.config_processing", library_config=library_config)
    ...
    for library_id, selected_collection_keys in library_config.items():
        ...
        logger.info("obs.zotero.library_syncing", library_name=library_name)
        ...
        if "ALL" in selected_collection_keys:
            ...
            logger.info("obs.zotero.collections_selected", collections_count=len(all_collections), selection="all")
        else:
            selected_collections = [
                col for col in all_collections if col["key"] in selected_collection_keys
            ]
            logger.info("obs.zotero.collections_selected", collections_count=len(selected_collections), selection="subset")
        ...
        for col in filtered_collections:
            try:
                ...
                if created:
                    ...
                    logger.info("obs.zotero.collection_created", collection=full_name)
                else:
                ...
                logger.error(
                    ...
                    collection=col.get("key", "unknown"),
                    error=str(e),
        ...
        logger.info("obs.zotero.children_fetching", items_count=len(items_to_sync))
        item_children_map = {}
        with ThreadPoolExecutor(max_workers=16) as executor:
            ...
            for future in as_completed(future_to_item):
                ...
                try:
                    ...
                    logger.error("obs.zotero.children_error", item_id=item["key"], error=str(e), error_type=type(e).__name__)
        ...
        logger.info("obs.zotero.pdfs_downloading", pdf_count=len(pdf_attachments))
        pdf_content_map = {}
        with ThreadPoolExecutor(max_workers=16) as executor:
            ...
            for future in as_completed(future_to_attachment):
                ...
                try:
                    ...
                    if content:
                        ...
                        logger.warning("obs.zotero.pdf_download_empty", attachment_id=child["key"])
                except Exception as e:
                    logger.error("obs.zotero.pdf_download_error", attachment_id=child["key"], error=str(e), error_type=type(e).__name__)
        ...
        logger.info("obs.zotero.pdfs_saving", pdf_count=len(pdf_attachments))
        for item, child in pdf_attachments:
            try:
                title = save_pdf_document(item, child)
                if title:
                    stats["pdfs_downloaded"] += 1
                    logger.info("obs.zotero.document_created", title=title)
            except Exception as e:
                logger.error("obs.zotero.document_save_error", attachment_id=child["key"], error=str(e), error_type=type(e).__name__)
                stats["errors"] += 1
    ...
    logger.info("obs.zotero.cleanup_start")
    ...
    for collection in collection_map.values():
        if not collection.documents:
            ...
            if not has_content:
                logger.info("obs.zotero.collection_deleted", collection=collection.name)
                collection.delete()
                empty_collections_deleted += 1

    stats["empty_collections_deleted"] = empty_collections_deleted

    logger.info("obs.zotero.sync_complete", username=user.username, stats=stats)
    return stats
