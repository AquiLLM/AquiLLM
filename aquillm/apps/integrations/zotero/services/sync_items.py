...
def sync_items_from_library(
    ...
    logger.info("obs.zotero.items_syncing", library_type=library_type)
    ...
    for item in items:
        try:
            ...
            if existing_doc:
                logger.debug("obs.zotero.item_skipped", item_id=item_key)
                continue
                ...
                if child_data.get("itemType") == "attachment":
                    ...
                    if "pdf" in content_type.lower() or child_data.get("filename", "").endswith(".pdf"):
                        ...
                        if PDFDocument.objects.filter(zotero_item_key=attachment_key).exists():
                            logger.debug("obs.zotero.pdf_skipped", attachment_id=attachment_key)
                            continue

                        logger.info("obs.zotero.pdf_downloading", title=title)
            ...
            logger.error(
                "obs.zotero.item_sync_error",
                item_id=item.get("key", "unknown"),
                error=str(e),
