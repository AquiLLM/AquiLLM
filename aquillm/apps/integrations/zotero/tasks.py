...
def sync_zotero_library(self, user_id: int, library_config: Optional[dict] = None):
    ...
    try:
        ...
        logger.error("obs.zotero.user_not_found", user_id=user_id)
        raise
    except ZoteroConnection.DoesNotExist:
        logger.error("obs.zotero.connection_not_found", user_id=user_id)
        raise
    except Exception as e:
        logger.error("obs.zotero.sync_error", error=str(e), error_type=type(e).__name__)
        raise
