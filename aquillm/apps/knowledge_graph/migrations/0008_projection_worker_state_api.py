...
BEGIN
  ...
  SELECT count(*)::integer INTO observed FROM public.apps_knowledge_graph_projectionchunkreference r JOIN public.apps_documents_textchunk c ON c.id=r.chunk_id AND c.id=r.integer_chunk_pk AND c.doc_id=r.document_uuid AND c.chunk_number=r.chunk_number WHERE r.projection_id=p_projection_id;
...
BEGIN
  PERFORM 1 FROM public.apps_collections_collection c WHERE c.id=p_collection_id FOR UPDATE; stable:=stable AND FOUND;
...
BEGIN
  PERFORM 1 FROM public.apps_collections_collection c WHERE c.id=p_collection_id FOR UPDATE; IF NOT FOUND THEN RETURN; END IF;
...
DO $$ BEGIN
 IF EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname='aquillm_projection_state') THEN
