...
def _golden_snapshot() -> ProjectedAuthorizedGraphSnapshotV1:
    snapshot = _snapshot(); evidence = ProjectedChunkEvidenceV1(K["fallback_chunk"], K["document_a"], 4, 0.25, K["fallback_provenance"]); mention = ProjectedIdentityMentionV1(K["target_identity"], evidence); fallback = ProjectedFallbackMentionAuditV1(0, K["target_identity"], evidence)
    return replace(snapshot, mentions=(mention,), audit_rows=(*snapshot.audit_rows[:2], fallback, *snapshot.audit_rows[2:]))
...
def test_public_stub_matches_runtime_dataclass_contract() -> None:
    ...
    enum_members = {"ProjectedEvidenceOrientationV1": {"HEAD_TO_TAIL": "head_to_tail", "TAIL_TO_HEAD": "tail_to_head"}, "ProjectedRetrievalDirectionV1": {"FORWARD": "forward", "REVERSE_DIRECTED": "reverse_directed", "UNDIRECTED": "undirected"}, "ProjectedScopeTypeV1": {"COLLECTION": "collection", "DOCUMENT": "document"}}
    for name in enum_names:
        assert "final" in {ast.unparse(item) for item in classes[name].decorator_list}; assert {item.targets[0].id: item.value.value for item in classes[name].body if isinstance(item, ast.Assign)} == enum_members[name]
...
def test_canonical_snapshot_vector_and_private_integer_canary_exclusion() -> None:
    ...
    assert projected_snapshot_checksum(snapshot) == "4b9536f1f41557ee3590c37403cbb00f87c7f7b136274fe51b9e520e7dc49923"
