...
class ClaudeInterface(LLMInterface):
    ...
    async def get_message(self, *args, **kwargs) -> LLMResponse:
        ...
        if isinstance(msgs, list) and sys_t is not None and mt is not None:
            ...
            _, new_max = apply_preflight_trim_to_message_dicts(
                str(sys_t), msgs, int(mt)
            )
        ...
        response = await self.client.messages.count_tokens(
            **(
                ...
                | {
                    ...
                    "messages": [
                        message.render(include={"role", "content"})
                        for message in messages_for_bot
                        + ([new_user_message] if new_user_message else [])
                    ],
                }
            )
        )
