...
def _init_tracing():
    ...
    try:
        ...
        logger.info("obs.observability.pyroscope_span_processor_attached")
    except Exception:
        logger.debug("obs.observability.pyroscope_otel_unavailable")
...
def _init_pyroscope():
    ...
    logger.info("obs.observability.pyroscope_init", server_address=server_address)
...
def setup():
    ...
    try:
        ...
        logger.exception("obs.observability.tracing_init_failed")

    try:
        _init_pyroscope()
    except Exception:
        logger.exception("obs.observability.pyroscope_init_failed")

    try:
        _init_pyroscope_celery_hook()
    except Exception:
        logger.exception("obs.observability.pyroscope_celery_hook_failed")

    try:
        _init_celery_user_tagging()
    except Exception:
        logger.exception("obs.observability.celery_user_tagging_failed")
