...
// See `vite.config.notes.ts` and `vite.config.ts` for why Monaco can't live
...
const CollectionNotesPage: React.FC<CollectionNotesPageProps> = ({
  ...
  const [data, setData] = useState<CollectionNote | null>(null);
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const updated = await saveCollectionNote(collectionId, body);
      setData(updated);
    ...
    <div className="p-[24px] md:p-[32px] flex flex-col h-full">
      <div className="mb-4">
        ...
        <p className="text-sm text-text-slightly_less_contrast mt-2 max-w-3xl">
          Markdown notes AquiLLM keeps in mind when answering questions about this
          collection. The notes are surfaced to the assistant alongside the documents it
          retrieves whenever this collection is selected in a conversation.
        </p>
      </div>
      ...
      <div className="flex-1 min-h-0 border border-border-low_contrast rounded mb-3">
        {loading ? (
          ...
          >
            <NoteEditor value={body} onChange={handleChange} />
          </Suspense>
        )}
      </div>
