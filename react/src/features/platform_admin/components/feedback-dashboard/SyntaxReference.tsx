...
| order by rating asc`}
        ...
        <div className="space-y-3 text-text-muted">
          <div>
            <p className="font-semibold text-text-normal mb-1">Available fields</p>
            <div className="font-mono grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-0.5">
              <span>rating</span><span className="text-text-muted/60">— 1–5 star rating</span><span></span>
              <span>feedback_text</span><span className="text-text-muted/60">— written comment</span><span></span>
              <span>feedback_submitted_at</span><span className="text-text-muted/60">— when rating/feedback was submitted</span><span></span>
              <span>model</span><span className="text-text-muted/60">— AI model used</span><span></span>
              <span>role</span><span className="text-text-muted/60">— user / assistant / tool</span><span></span>
              <span>content</span><span className="text-text-muted/60">— message text</span><span></span>
              <span>created_at</span><span className="text-text-muted/60">— message timestamp</span><span></span>
              <span>user_id</span><span className="text-text-muted/60">— user account ID</span><span></span>
              <span>conversation_id</span><span className="text-text-muted/60">— conversation ID</span><span></span>
              <span>sequence_number</span><span className="text-text-muted/60">— position in conversation</span><span></span>
              <span>message_uuid</span><span className="text-text-muted/60">— unique message ID</span><span></span>
              <span>tool_call_name</span><span className="text-text-muted/60">— tool used (intermediate messages only)</span><span></span>
              <span>conversation_tool</span>
              <span className="text-text-muted/60 col-span-2">— filter-only: did this conversation use a tool? (see below)</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1 font-mono">
              <div><span className="text-text-normal">messages</span> — every query must start with this</div>
          ...
          <div>
            <p className="font-semibold text-text-normal mb-1">
              Filtering by tool use (<code>conversation_tool</code>)
            </p>
            <p className="text-text-muted mb-1.5 leading-relaxed">
              ...
              <code className="text-text-normal">conversation_tool</code> bridges this gap — it
              checks whether the conversation as a whole involved a particular tool. Use it in{' '}
              <code className="text-text-normal">where</code> clauses only.
            </p>
            <div className="font-mono space-y-0.5">
              <div><span className="text-text-normal">conversation_tool == &quot;vector_search&quot;</span> <span className="text-text-muted/60">— conversation used vector search</span></div>
              <div><span className="text-text-normal">conversation_tool != &quot;vector_search&quot;</span> <span className="text-text-muted/60">— conversation did not use vector search</span></div>
              <div><span className="text-text-normal">conversation_tool != null</span> <span className="text-text-muted/60">— conversation used any tool</span></div>
              <div><span className="text-text-normal">conversation_tool == null</span> <span className="text-text-muted/60">— conversation used no tools</span></div>
            </div>
