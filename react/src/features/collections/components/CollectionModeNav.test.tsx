...
describe("CollectionModeNav", () => {
  it("shows Files selected by default", () => {
    ...
    expect(
      screen
        .getByRole("tab", { name: "Knowledge Graph" })
        .getAttribute("aria-selected"),
    ).toBe("false");
    expect(
      screen
        .getByRole("tab", { name: "Visualization" })
        .getAttribute("aria-selected"),
    ).toBe("false");
  });
  ...
  it("deep-links to visualization and writes its query value when selected", () => {
    const pushState = vi.spyOn(window.history, "pushState");
    const onActiveModeChange = vi.fn();
    setLocation("/collections/7", "?tab=details&view=visualization");
    ...
    expect(pushState).toHaveBeenCalledWith(
      null,
      "",
      "/collections/7?tab=details&view=visualization",
    );
    expect(onActiveModeChange).toHaveBeenCalledWith("visualization");
  });

  it("falls back to Files for invalid view values via parent state", () => {
    setLocation("/collections/7", "?view=invalid-mode");
    render(
      <CollectionModeNav activeMode="files" onActiveModeChange={vi.fn()} />,
    );
    expect(
      screen.getByRole("tab", { name: "Files" }).getAttribute("aria-selected"),
    ).toBe("true");
  ...
  it("updates URL with pushState when switching modes", () => {
    ...
    expect(pushState).toHaveBeenCalledWith(
      null,
      "",
      "/collections/7?tab=details&view=knowledge-graph",
    );
  ...
  it("blocks navigation when guardNavigation returns false", () => {
    ...
    expect(guardNavigation).toHaveBeenCalledWith({
      type: "mode",
  ...
  it("reverts browser navigation when guardNavigation rejects popstate", () => {
    ...
    const guardNavigation = vi.fn(
      ({ mode }: { mode: CollectionViewMode }) => mode !== "knowledge-graph",
    );
