...
`answer_sha256` (SHA256 of exact UTF-8 answer), `subject` copied exactly from the
observation `review_subject`, `answer_faithful`,
...
re-score immutable observations without calling providers again. The subject schema is
`evidence-review-subject-v1` with `observation_sha256` and `answer_sha256`. It
binds explicit original case/split/mode/run/repetition/invocation/profile fields,
code revision, source/runtime/configuration snapshots, resolved treatments, answer, delivered spans,
source bindings, final SDK payloads, trace and observation status. Derived scores and
reviews are excluded, so rescoring is stable. Each actual turn has a fresh invocation
ID; operational run IDs retain the predeclared schedule. Copy the subject only after
reviewing that exact observation. Missing/stale/malformed/model judgments stay unknown.
Both valid and rejected submitted records remain in `human_review`; `review_valid`
records validation. Comparisons and operational attachments revalidate the record.

For a runnable review workflow, first run and save the report, inspect its exact
answer/evidence/trace, create a JSON object keyed by case ID (operational: run ID),
and copy `review_subject` into each reviewed record `subject`. Supply the named
human labels above, then use the same runner/mode/split/profile/repetition arguments
with `--observations saved-report.json --reviews reviews.json --report reviewed.json`.
Subsequent rescoring can omit `--reviews` to revalidate retained records. No provider
call is made by this path and no human labels are generated by the runner.
Supported SDK Pydantic models (including actual Gemini Content/Part/config),
dataclasses, UUIDs, enums, tuples and JSON values use one canonical JSON-safe
normalization before trace/subject/partial assessment and report writing. All shaped
values stay bound; unsupported representations mark observation incomplete and cannot
produce an eligible review subject. Initial and saved/rescored subjects are identical.

Live mode validates the production-resolved selector and preservation settings before
seeding or turn execution; inherited invalid selector shadow settings are rejected.
`resolved_treatment` records intended arm differences separately from common effective
`comparison_controls`: selector timeout, rerank provider/endpoint/model/pair limits,
timeouts/concurrency/document limit, consumed connect/receive outer limits, tool limits,
prompt packing settings and cache controls. Request timeout/retry/stream getters are
shared with OpenAI dispatch. Common completion controls also include post-tool and
whole-document output/continuation caps, direct/tool/general retry caps, citation
repair input bound, synthesis retries and actual lease allowances, cutoff/fallback/
evidence-retry policies, final-answer-only publication, citation enforcement/source
appending and the resolved outer tool-loop policy. They use the same production
getters as orchestration, preserving existing defaults. Endpoint credentials and API keys are never recorded.
Snapshot comparison rejects effective-control drift. Observer callback/copy failures
are isolated from application work and mark the observation incomplete/ineligible.
...
Operational human reviews are keyed by exact run ID and bind the same canonical
subject (including answer SHA, profile, repetition and actual invocation). Refinement
...
`usable_fact_count` and `retained_fact_count` checked against concrete records.
`available_facts` is a list of unique `fact_id`, exact `span` (source_id, revision,
fingerprint, start/end Unicode codepoints, text) and `acquisition_event` trace index.
Each retained fact additionally has `answer_start`, `answer_end`, the exact `citation`
inside that answer portion, and human `entailed`/`qualifications_correct` booleans.
`retained_fact_ids` names these records. The source must be currently authorized,
observed in that acquisition event, and the retained span must map to the actual
final SDK packet through recorded source bindings. `limit_answer_span` contains
start/end/text for the nonempty explicitly bounded response reviewed by the human.
`losses` identifies every unretained fact with `fact_id`, final `sdk_event` index,
`reason` and `reviewed: true`; its absence must be observed in the final packet.
Each loss also requires `cause: {type: selection_limit, event: INDEX, resource: passages}`
with resource `passages`, `document_passages` or `evidence_tokens`. The referenced
`selection_excluded` event must be after acquisition and before the final SDK event,
in the final-selection phase, and identify that exact prepared source fingerprint
and containing Unicode span. Its observed used/required/limit counters must prove
the named resource prevented admission. The selector emits this opt-in event from
its actual remaining candidates; it changes no selection decisions. Free-form
`reason` is a human explanation, never machine proof of a different cause. Missing,
unrelated, wrong-phase/source/span or contradictory resource references block proof.
Authorization denial and other unsupported cause types remain unproven; a global
exhaustion elsewhere cannot stand in for a fact-specific exclusion.
Zero usable facts additionally requires `empty_reason`, nonempty `empty_event_refs`
pointing to actual empty retrieval events, empty upstream/delivery/citations and no
answer citation. A free-form explanation, empty answer or asserted counts alone
cannot pass. Semantic entailment remains a named-human judgment.
...
acquisition-pair-reserve OR retrieval-deadline exhaustion in the `pilot` profile.
`one_action` results remain visible diagnostics and cannot discharge the pilot subtype.
Action/source limits,
