...
**Architecture:** Add a unified ingestion service + type detector + parser registry. Expose a single multipart API endpoint (`/api/ingest_files/`) used by a new “Upload documents” modal. Refactor Zotero sync/manual import and arXiv ingestion to call the same service. Persist new uploads into new generic `DocumentV2`/`DocumentAsset` models while keeping legacy models readable.
...
Full behavior is in **`docs/plans/2026-03-16-auto-ingest-design.md`** under **“Caveats and edge cases”**. Summary for implementation:
