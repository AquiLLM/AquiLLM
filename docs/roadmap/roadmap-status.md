...
| Bigger vLLM model configuration | `2026-03-16-bigger-vllm-models.md` | **Partial / drifted** | Larger-model config pattern exists, but current `.env`/`.env.multimodal` differ from plan target model IDs and context lengths | Reconcile desired model targets vs current cost/perf constraints |
