...
|---|---|---|---|---|
| RAG tool-calling reliability + direct RAG pipeline | `2026-05-31-rag-reliability-research-upgrade.md` | **Implemented (Tasks 0–8 complete)** | `apps/chat/services/rag_pipeline.py`, `rag_metrics.py`, `rag_intent.py`, `rag_evidence.py`, `rag_synthesis.py`; `lib/llm/providers/tool_budget.py`, `complete_turn.py`; offline eval harness `apps/chat/evals/`; 63+ test-backed | Live A/B + production telemetry review pending |
...
| Optional WhisperX enhancement | `docs/specs/2026-03-30-whisperx-transcription-design.md`, `docs/roadmap/plans/pending/2026-03-30-whisperx-transcription-implementation.md` | **Not started** | Configured OpenAI-compatible ASR baseline exists with Nemotron default and documented Whisper rollback | Implement the optional enhancer without changing the baseline contract |
