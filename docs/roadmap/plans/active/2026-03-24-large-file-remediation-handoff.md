# Large-file remediation â€” continuation handoff (2026-03-24)
...
## Commits on `development` (reference â€” verify with `git log`)
...
| Plan | Short hash | Subject |
|------|------------|---------|
| **18â€“21** | `8e0d1a5` | `refactor(frontend): move chat, collections, documents, and admin UI to features` |
| **22â€“23** | `1693dd5` | `docs: document React features layout; chore(structure): refresh file-length allowlist` |
...
### OpenAI provider â€” token / overflow modules

| Module | Role |
|--------|------|
| `aquillm/lib/llm/providers/openai_tokens.py` | Env helpers, `context_reserve_tokens`, flatten/estimate, `trim_messages_for_overflow`, `preflight_trim_for_context(cls, â€¦)` |
...
### Chat tests â€” `test_messages.py` removed

| File | Role |
|------|------|
| `aquillm/apps/chat/tests/chat_message_test_support.py` | Shared fakes + `@llm_tool` stubs â€” not a test module |
| `test_message_adapters.py` | Pydantic â†” Django adapters + `build_frontend_conversation_json` |
...
### React â€” `features/*` layout (commits 18â€“21)
...
### Docs + structure (commits 22â€“23)

- **README.md** â€” â€œModule layoutâ€ includes a bullet on `react/src/features/*` and shim pattern.
- **`scripts/check_file_lengths.py`** â€” Allowlist updated: removed shims and slimmed `IngestRowsContainer`; **still allowlists** the three large feature files below plus `CollectionsPage.tsx` and all prior backend hotspots.
...
1. **Circular import (`tool_wiring` â†” `ChatConsumer`)** â€” `apps/chat/consumers/__init__.py` uses **`__getattr__`** for lazy `ChatConsumer`. Do not eager-import `chat.py` at package init without fixing the cycle.
2. **`lib` must not import `apps.*`** â€” run `python scripts/check_import_boundaries.py` after Python refactors.
3. **Wiring type hints** â€” `ChatConsumer` stays forward-quoted in `tool_wiring` where needed.
4. **React path depth** â€” Files under `features/<domain>/components/` typically need **`../../../`** to reach `src/components`, `src/types`, `src/utils`. Cross-feature imports use `../../other_feature/...` (e.g. collections â†’ documents).
...
| **Backend allowlist** | When `chat.py`, `base.py`, or `openai.py` drops â‰¤300 lines, remove only that path from `_ALLOWLIST` in `scripts/check_file_lengths.py` (proof: script exit 0). |
| **React &gt;300** | Split `CollectionView`, `FileSystemViewer`, and `UserManagementModal` until each is â‰¤300 lines, then remove their paths from the allowlist. |
...
- [2026-03-19-aquillm-structure-code-quality-remediation.md](./2026-03-19-aquillm-structure-code-quality-remediation.md) â€” chat append regression tests, ingest consumer auth, settings/toolbar security, logging cleanup, **`main.tsx` barrel imports**, **`IngestRow` dedup**, `.gitignore` / hygiene CI.
...
**Backend â€” full chat + LLM (needs Postgres reachable; use Compose if `POSTGRES_HOST=db`):**
