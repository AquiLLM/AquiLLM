# Feedback rating CSV export â€” execution notes

Implementation follows `docs/roadmap/plans/active/2026-03-21-feedback-rating-csv-export.md`.

## Schema

- `Message.feedback_submitted_at` â€” set when a client sends `rate` or `feedback` over the chat WebSocket (validated rating 1â€“5; feedback text truncated to 10k chars).

## Backend

- **Export logic:** `aquillm/apps/platform_admin/services/feedback_export.py` â€” queryset, `question_number` via subquery count of `role='user'` messages with `sequence_number <=` the assistant row.
- **CSV view:** `aquillm/apps/platform_admin/views/api.py` â†’ `feedback_ratings_csv`, URL name `api_feedback_ratings_csv`, mounted at `GET /api/feedback/ratings.csv` in `aquillm/aquillm/api_views.py`.
...
Use these if you want history to match the planâ€™s tasks. Paths include a few extras that were not listed in the plan but are part of this feature.
...
**2 â€” WebSocket feedback capture**
