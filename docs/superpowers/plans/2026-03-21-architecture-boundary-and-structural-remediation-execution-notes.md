# Architecture boundary remediation — execution notes
...
### Runtime entry points (commits 1–2)
...
### Documents / compatibility layer (commits 3–4)
...
- `docs/architecture/aquillm-current-architecture-mermaid.md`: ASGI routing note.
...
- **Commits 5–10**: Split `apps/chat/consumers/chat.py`, extract `lib/tools` search/document modules with `tool_wiring`, split `lib/llm/providers/base.py` and `openai.py`, decompose `apps/chat/tests/test_messages.py`.
...
- **Commits 14–15**: Move large React components under `react/src/features/*` with re-exports from `components/`.
